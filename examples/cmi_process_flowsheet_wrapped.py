...
def set_operation_conditions(ctx: Context):
    """
...
def set_scaling(ctx: Context):
    ...
    ctx["unscaled_model"] = m
    ctx["scaling"] = scaling
    ctx["diagnostics"] = dt
    ctx.model = scaled_model


@FS.step("initialize")
def initialize_system(ctx: Context):
    """
    Initialize system.
...
def solve_system(ctx: Context):
    ...
    # Solve flowsheet
    results = ctx.solver.solve(m, tee=ctx["tee"])
    ...
    ctx["diagnostics"].assert_no_numerical_warnings()

    res = ctx["scaling"].propagate_solution(m, ctx["unscaled_model"])

    display_results(ctx["unscaled_model"])

    ctx["propagated_results"] = res
